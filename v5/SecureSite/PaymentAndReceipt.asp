...
'**************************************************************
Dim vPostData
vPostData = "x_login=" & x_login & "&x_tran_key=" & x_tran_key & "&x_version=" & x_version & "&x_method=" & x_method
vPostData = vPostData & "&x_test_request=" & x_test_request &"&x_delim_data=" & x_delim_data & "&x_delim_char=" & x_delim_char & "&x_relay_response=" & x_relay_response
vPostData = vPostData & "&x_first_name=" & x_first_name & "&x_last_name=" & x_last_name & "&x_address=" & x_address & "&x_city=" & x_city & "&x_state=" & x_state & "&x_zip=" & x_zip & "&x_country=" & x_country & "&x_phone=" & x_phone 
vPostData = vPostData & "&x_cust_id=" & x_cust_id & "&x_email=" & x_email &"&x_email_customer=" & x_email_customer 
vPostData = vPostData & "&x_invoice_num=" & x_invoice_num &"&x_description=" & x_description 
vPostData = vPostData & "&x_amount=" & x_amount & "&x_currency_code=" & x_currency_code & "&x_method=" & x_method & "&x_type=" & x_type &"&x_recurring_billing=" & x_recurring_billing 
vPostData = vPostData & "&x_card_num=" & x_card_num & "&x_exp_date=" & x_exp_date 
vPostData = vPostData & "&x_card_code=" & x_card_code & "&x_trans_id=" & x_trans_id &"&x_auth_code=" & x_auth_code 

''''vPostData = vPostData & "x_line_item=item1<|>Tickets<|>A1,A2,A3<|>3<|><|>N&"
...
if bTest then
		    ...
		    Transaction_id="LHD TEST"
else if cdbl(x_amount) < 0.10 then
    		Response_Code="1"
		    Reason_Code="1"
		    Reason_Text="ACCEPTED"
		...
		dim Transaction_id
		       ...
		       for i=0 to ubound(arrData)
					...
					Select Case iq
					    case 1
					        Response_Code=arrData(i)
					    case 3
					        Reason_Code=arrData(i)
					    case 4
					        Reason_Text=arrData(i)
					    case 5
					        Authorization_Code=arrData(i)
					    case 7
					        Transaction_id=arrData(i)
