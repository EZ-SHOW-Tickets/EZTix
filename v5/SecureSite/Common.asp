...
'''Theater Information
dim TheaterID,TheaterName,TheaterAddress,TheaterCity,TheaterState,TheaterZip,TheaterLabelType,TheaterMapQuest
...
''connection_string="DRIVER={SQL Server};SERVER=209.204.62.15;DATABASE=CGCTicketing;UID=sa;PWD=cgcuser"
''connection_string= "DSN=CGCTicketing;uid=sa;pwd=cgcuser;"
''' General Information
dim DayOfWeek(7)
dim isTTO
DayOfWeek(1)="Sunday"
...
function GetRecordSet(strSQL)
	...
	if TestInput(strSQL) then
	    Response.Redirect "SQLErrorPage.asp"
		exit function
	else
...
function oldInsertRecord(strSQL,InsertTable,NewID)
	...
	'oCommand.CommandText = "SELECT @@IDENTITY"
	Set oRS = oCommand.Execute()	
	InsertRecord=oRS(0)
	set m_oConnection=nothing
	set oCommand=nothing
...
function GetSeatCategoriesForATheater(TheaterID)
	...
	strSQL="Select distinct THEATER_SEAT_CATEGORY.SeatCategoryID, SeatCategory from THEATER_SEAT_CATEGORY,SEAT_CATEGORY where THEATER_SEAT_CATEGORY.SeatCategoryID=SEAT_CATEGORY.SeatCategoryID and TheaterID=" & TheaterID
	set oRs=getrecordset(strSQL)
...
function GetAllPerformanceForShow(ShowID)
	'set oRS=getrecordset("Select * from PERFORMANCES where ShowID = " & ShowID & " order by ShowDate,ShowTime")
...
function GetPurchasedSeatsForAPerformancesForPerson(PerformanceID,ReservedFor)
	strSQL="SELECT seatNumber,CostBasis,PaymentID"
	strSQL=strSQL & " FROM SHOW_SEATING"
...
function TestInput(StringToSearch)
	...
	.Pattern = "<|--|drop |delete |xp_|up_|sp_|script|\|;"
	.IgnoreCase = True
	.Global = True
	End With
