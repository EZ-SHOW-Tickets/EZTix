...
'	session("TheaterID")=TheaterID
	...
	NumTea=0
	NumHC=0
	if session("AccountID")="3" then
		NumTea=getFromRecord("SELECT Count(SHOW_SEATING.SeatNumber) AS CountOfSeatNumber FROM SHOW_SEATING WHERE SHOW_SEATING.CostBasis=6  AND SHOW_SEATING.PerformanceID=" & PerformanceID)
	elseif session("AccountID")="5" then
		NumHC=getFromRecord("SELECT Count(SHOW_SEATING.SeatNumber) AS CountOfSeatNumber FROM SHOW_SEATING WHERE SHOW_SEATING.CostBasis=169  AND SHOW_SEATING.PerformanceID=" & PerformanceID)
	end if
	...
	if Request.QueryString.Count > 0 then
		...
		for i = 1 to Request.QueryString.Count
			if Request.QueryString.key(i)<>"NumberOfClicks" then
				...
				'''''
				'See if these are HC seats that were picked
'				set rsHC = getrecordset("select * from SHOW_SEATING where SeatNumber='" & SeatNumber & "' and CostBasis=130 and PerformanceID=" & PerformanceID)
'			    if not rsHC.eof then
'					'This is set aside as HC... DELETE IT
'					deleterecord "DELETE FROM SHOW_SEATING where ShowID=" & ShowID & " and PerformanceID=" & PerformanceID & " and SeatNumber='" & SeatNumber & "'"							
'			    end if
				...
				'Seats are available
					...
					Response.Redirect "PlumTheaterSeating.asp?ErrMessage=WE ARE SORRY! One or more of these seats are currently reserved by another patron and are pending purchase completion. Please select alternate seats."
				end if
			end if
		next
		...
		for i=1 to ubound(Seats,2)
			...
			strCost= strCost & "<tr>"
			strCost= strCost & "<td width='420' height='6' colspan='3'>"
			strCost= strCost & "<b><font face='Verdana' size='2'>You MUST show your certificate to receive the following number of seats:</font></b></td>"
			strCost= strCost & "</tr>"
			strCost= strCost & "<tr>"
			strCost= strCost & "<td width='420' height='6' colspan='3'>"
			...
			do until rsPrices.eof
				...
				'	strCost= strCost & "<td width='50'><input type='hidden' name='PriceCategoryType" & j & "' value='" & rsPrices("PriceCategoryID") & "'><input name='PriceCategory" & j & "' size='2' maxlength='2' value='0' readonly></td>"
				'	strCost= strCost & "<td width='342'><b><font face='Verdana' size='2'>TEA SOLD OUT</font></b></td>"
				''elseif session("AccountID")="5" and numTEA > 4  and rsPrices("PriceCategoryID")=169 and session("Admin")="0" then
				''	strCost= strCost & "<td width='50'><input type='hidden' name='PriceCategoryType" & j & "' value='" & rsPrices("PriceCategoryID") & "'><input name='PriceCategory" & j & "' size='2' maxlength='2' value='0' readonly></td>"
				''	strCost= strCost & "<td width='342'><b><font face='Verdana' size='2'>HC Unavailable</font></b></td>"
				''else	
					strCost= strCost & "<td width='50'><input type='hidden' name='PriceCategoryType" & j & "' value='" & rsPrices("PriceCategoryID") & "'><input name='PriceCategory" & j & "' size='2' maxlength='2' value='" & totalSeats & "' readonly></td>"
					strCost= strCost & "<td width='342'><b><font face='Verdana' size='2'>" & rsPrices("PriceCategory") & "</font></b></td>"
				''end if
				strCost= strCost & "</tr>"
				rsPrices.movenext
			loop
...
<body onLoad="MM_preloadImages('images/Submit2ON.gif','images/CancelTicketsON.gif')">
<script language="javascript"> 

function GoBack(){
//document.location='SelectPerformance.asp';
//document.location='PlumOrder.asp?ReleaseSeats=1';
          						...
          						<td width="420" valign="top" align="left" height="115">
            						<table border="0" width="420" height="291">
                      					...
                      					<!--				<td width="120" height="1" valign="top" align="left" colspan="1">                      										<font face="Verdana" size="2"><b>Performance:</b></font>                      									</td>-->
                                      					...
                                      					<form name="fTickets" method="POST" action="ProcessPlumSeats.asp">
                     								...
                     								<tr>
                      								...
                      								<td width="420" height="6" colspan="2">
														...
														<a href="javascript:PurchaseTickets(<%=j%>,<%=totalSeats%>,0,<%=sType%>);" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image0','','images/Submit2ON.gif',1)"><img name="Image0" border="0" src="images/Submit2.gif" WIDTH="124" HEIGHT="32"></a>
                                            		</td>
                    								</tr>
                    								<!--<tr>                      								<td width="420" height="6" colspan="2">                      									<p align="center">                      									<a href="javascript:PurchaseTickets(<%=j%>,<%=totalSeats%>,1,<%=sType%>);" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image2','','images/SelectAnotherPerfON.gif',1)"><img name="Image2" border="0" src="images/SelectAnotherPerf.gif" WIDTH="350" HEIGHT="32"></a>                                            		</td>                    								</tr>-->
                    								<tr>
