	...
	do until rsPerf.eof
			if session("Admin") <> "0" or datediff("h",now(),(rsPerf("ShowDate") & " " & rsPerf("ShowTime"))) > ShowHoldHours then
				...
				if (rsPerf("PerformanceOpen")=0 or session("Admin")="1") and totalSeatsAvailable > 0 then
					...
					strPerf=strPerf & "<tr><td width='24' align='right'><font face='Arial' size='2'><input type='radio' name='sPerformance' id='sPerformance' value='" & rsPerf("PerformanceID") & "' onclick='javascript:SelectPerformanceNew(" & j & ");'></font></td>"
					j=j+1
				else
					'Sold Out
...
'Clear Recordsets
set rsPerf=nothing
Response.Redirect "ProcessFreePerformance.asp?PerformanceID=7183"
%>
<html>
...
function SelectPerformanceNew(n)
{
	...
	{document.location="ProcessFreePerformance.asp?PerformanceID="+document.fSelectPerformance.sPerformance.value;}
	else
	{document.location="ProcessFreePerformance.asp?PerformanceID="+document.fSelectPerformance.sPerformance[n].value;}
}
...
if (show==0) {alert("Please select a performance date");}
	else{document.location="ProcessFreePerformance.asp?PerformanceID="+show;}				 
}
	...
	<tr>
		<td width="100%" height="400" align="middle">
			...
			<table border="0" width="600" height="16">
  				<tr>
    				<td width="600" height="12"><%=strShows%><p align="center"><!--<input type="button" value="SELECT A DIFFERENT SHOW" onclick="javascript:GoBack();"></td>-->
          						...
          						<td width="60%" valign="top" align="left" height="115">
            						<table border="0" width="100%" height="291">
              							...
              							<tr>
                                            ...
                                            <br>Ticket Prices:</b><%=strPriceRange%></font>

                					<br><font face="verdana,arial,helvetica,sans-serif" size="1" color="#000080"><b>On-Line Sales close <%=ShowHoldHours%> hours before performance</b></font>
                  					<!--<p></p>-->
                  				</td>
