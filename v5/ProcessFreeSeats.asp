...
<% 
	...
	if Request.Form("MoreTicks")="0" then
		Response.Redirect "https://www.cgc-services.com/eztv5/SetupFreetickets.asp?PerfomanceID=" & PerformanceID & "&AccountID=" & session("AccountID") & "&Admin=" & session("Admin") & "&lSessionID=" & session("lSessionID")	
		''''''''TEST MODE
		'Response.Redirect "SecureSite/SetupFreetickets.asp?PerfomanceID=" & PerformanceID & "&AccountID=" & session("AccountID") & "&Admin=" & session("Admin") & "&lSessionID=" & session.sessionID	
		''''''''''''''''
	else
